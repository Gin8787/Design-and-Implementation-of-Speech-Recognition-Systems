...
bool WriteWave(const char *wavFile, short *waveData, int numSamples, int sampleRate)
{
	FILE	*wavFp;
	WavFileHead		wavHead;
	long	numWrite;
	bool isFileSaveSuccess = false;
...
//	cout << "data length is " << (File_length - sizeof(struct WavFileHead)) / sizeof(short) << endl;
	printf("data length is %ld\n", File_length - sizeof(struct WavFileHead) / sizeof(short));

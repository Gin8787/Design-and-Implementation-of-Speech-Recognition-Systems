...
void feature_extraction(short *waveData, int nbSamples, int sampleRate, string normalFileName)
{
	printf("-------------------In MFCC Now------------------\n");
	int i, j;
	int nbFrames = 0;
	...
	ofstream normalization(normalFileName.c_str());
	//ofstream org("original.txt");
...
//	FILE *melspecfile = fopen("melspecfile.txt", "w");
	...
	/**Calculate Frames**/
	nbFrames = ceil((double)((nbSamples - FRAME_LENGTH) / (FRAME_LENGTH - OVERLAP))) + 2; //Quest?
